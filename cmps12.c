...
bool read_raw_data(RawData* data) {
    ...
    if (i2c_write_blocking(I2C_PORT, CMPS12_ADDR, &reg, 1, true) != 1) return false;
    if (i2c_read_blocking(I2C_PORT, CMPS12_ADDR, buffer, 18, false) != 18) return false;
...
bool read_navigation_data(NavData* data) {
    ...
    if (i2c_write_blocking(I2C_PORT, CMPS12_ADDR, &reg, 1, true) != 1) return false;
    if (i2c_read_blocking(I2C_PORT, CMPS12_ADDR, buffer, 4, false) != 4) return false;
...
bool check_calibration(CalibData* data) {
    ...
    if (i2c_write_blocking(I2C_PORT, CMPS12_ADDR, &reg, 1, true) != 1) return false;
    if (i2c_read_blocking(I2C_PORT, CMPS12_ADDR, &status, 1, false) != 1) return false;
